...
class EmailProcessor:
    ...
    def __init__(self):
        ...
        self.azure_openai_deployment_chat = os.getenv("AZURE_OPENAI_DEPLOYMENT_CHAT", "gpt-4")
        self.azure_openai_deployment_emb = os.getenv("AZURE_OPENAI_DEPLOYMENT_EMBEDDING", "text-embedding-3-small")
    ...
    def _detect_embedding_deployment(self):
        ...
        # 모든 배포명 실패시 오류
        raise ValueError("사용 가능한 임베딩 배포를 찾을 수 없습니다. AZURE_OPENAI_DEPLOYMENT_EMB 환경 변수를 확인하세요.")
    ...
    def preprocess_email(self, email_data: Dict) -> Dict:
        ...
        standardized = {
            ...
            "from": {
                "name": safe_get("from_name"),
                "email": safe_get("from_address")
            },
            "to": to_list,
            "cc": cc_list,
    ...
    def analyze_with_policy_engine(self, email_data: Dict, user_context: Dict) -> Dict:
        ...
        try:
            if to_contains_self and request_detected:
                # A: To에 포함 + 요청 표현
                user_mentions = [mention for mention in mentions if user_name and f"@{user_name}" in mention]
                if not any(mention for mention in mentions if mention != f"@{user_name}") or user_mentions:
                    policy_decision = "A"
            elif cc_contains_self and not to_contains_self:
                # B: CC에만 포함
                if any(user_name and f"@{user_name}" in mention for mention in mentions):
                ...
                # C: 본인이 보낸 요청
                policy_decision = "C"
            elif to_contains_self and user_team and user_team in body and request_detected:
                # D: 팀 단위 요청
                policy_decision = "D"
        except Exception as e:
    ...
    def extract_actions_with_llm(self, email_data: Dict, policy_signals: Dict, user_context: Dict) -> Dict:
        """LLM을 사용한 액션 추출"""
...
"""
        ...
        try:
            ...
            logging.info(f"✅ LLM 액션 추출 완료: {result.get('is_action', False)}")
            return result
            
        except Exception as e:
            logging.error(f"❌ LLM 추출 실패: {e}")
    ...
    def normalize_action(self, raw_action: Dict, email_data: Dict) -> Optional[Dict]:
        ...
        # 날짜 정규화
        due_date = ""
        due_raw = action.get("due_raw", "")
        if due_raw:
            # 간단한 날짜 변환 (실제로는 더 정교한 파싱 필요)
            today = datetime.now(timezone.utc).date()
            if "금일" in due_raw or "오늘" in due_raw:
                due_date = today.isoformat()
            elif "내일" in due_raw:
                due_date = (today.replace(day=today.day + 1)).isoformat()
        
        # 담당자 결정
        assignee = "미지정"
        candidates = action.get("assignee_candidates", [])
        if candidates:
            # 이메일 주소가 포함된 첫 번째 후보 선택
        ...
        if action.get("type") == "DO" and due_raw and "@" in assignee:
            confidence += 0.2
    ...
    def create_text_chunks(self, text: str, chunk_size: int = 900, overlap: int = 150) -> List[str]:
        """텍스트 청킹"""
    ...
    def get_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            response = self.openai_client.embeddings.create(
                model=self.azure_openai_deployment_emb,
                input=texts
            )
    ...
    def upload_to_search(self, email_data: Dict, action_data: Optional[Dict]) -> None:
        ...
        for i, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
            ...
            document = {
                ...
                "chunk": chunk,
                "bodyPreview": email_data['body'][:200] + "..." if len(email_data['body']) > 200 else email_data['body'],
            ...
            if action_data:
                document.update({
                    ...
                    "due": action_data.get('due', '') + "T00:00:00Z" if action_data.get('due') else None,
        ...
        for item in emails:
            try:
                ...
                if not isinstance(email_data, dict):
                    logging.warning(f"⚠️ data 필드가 딕셔너리가 아닌 레코드 건너뜀: {record_id}")
                    continue
                ...
                if missing_fields:
                    logging.warning(f"⚠️ 필수 필드 누락으로 건너뜀 {record_id}: {missing_fields}")
                    continue
                ...
                # 3. 정책 엔진 적용
                policy_signals = self.analyze_with_policy_engine(email_data, user_context)
                ...
                if action_result.get("is_action"):
                    normalized_action = self.normalize_action(action_result, standardized_email)
...
def main():
    ...
    try:
        ...
        # 이메일 파일 경로
        email_file = "../data/email_sample_2.json"
